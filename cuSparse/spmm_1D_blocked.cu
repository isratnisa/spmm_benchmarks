...
int main(int argc, char **argv)
{
    ...
    int **b_rowInd = (int **)malloc(rootp * sizeof (int *) );
    ...
    for (int b = 0; b < rootp; ++b) {tot_nnz += nnzBlock[b];
    printf("nnzBlock %d %d\n", nnzBlock[b], tot_nnz);}
    ...
    for (int br = 0; br < rootp; ++br) //loop over row blocks of C 
    {
        ...
        {         
            ...
            status = cusparseCreateCsr(&a_cusparse, nrows, ncols, nnz,
                            d_rowPtr, d_colInd, d_vals, 
                            CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                            CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F);
       
            cusparseDnMatDescr_t b_cusparse;
            status = cusparseCreateDnMat(&b_cusparse, ncols, b_rhs, b_rhs,
                              cy_in, CUDA_R_32F, CUSPARSE_ORDER_ROW);

            cusparseDnMatDescr_t c_cusparse;
            status = cusparseCreateDnMat(&c_cusparse, nrows, b_rhs, b_rhs,
